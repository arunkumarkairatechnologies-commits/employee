...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_group'])) {
    ...
    if (!empty($group_name) && !empty($members)) {
        ...
        foreach (array_unique($members) as $mem_id) {
            $conn->query("INSERT INTO chat_group_members (group_id, user_id) VALUES ($group_id, $mem_id)");
        }
        header("Location: chat.php?type=group&id=$group_id");
        exit;
    }
}
...
?>
<style>
/* Modern Chat UI mimicking the user design */
body {
...
.chat-sidebar-header h3 i:hover {
            ...
            <div class="chat-app-wrapper">
                ...
                <div class="chat-sidebar">
                    <div class="chat-sidebar-header">
                        <h3>Messages <a href="#" data-bs-toggle="modal" data-bs-target="#createGroupModal" class="text-primary fs-5 ms-3" title="Create Group"><i class="fas fa-users"></i></a></h3>
                    ...
                    <div class="chat-list">
                        ...
                        <!-- All Employees -->
                        <div class="chat-list-section d-flex align-items-center mt-2 mb-2">
                            <i class="far fa-comment-dots me-2"></i> All Employees
                        </div>
                        <?php
                        ...
                        if ($groups) {
                            while ($g = $groups->fetch_assoc()) {
                                ...
                                echo '  </div>';
                                echo '  <div class="chat-item-content">';
                                echo '      <div class="chat-item-title"><span>' . htmlspecialchars($g['name']) . '</span></div>';
                                echo '      <div class="chat-item-subtitle">';
                ...
                <div class="chat-main">
                    <?php if ($active_id > 0): 
                        ...
                        if ($chat_type === 'user') {
                            ...
                            if ($u_info) {
                                ...
                                $color = getUserColor($chat_name);
                                $header_avatar = "https://ui-avatars.com/api/?name=".urlencode($chat_name)."&background=".substr($color,1)."&color=fff";
                            }
                        } else {
                    ...
                    <div class="chat-header">
                        <div class="chat-header-info">
                            ...
                            <div>
                                ...
                                <?php if ($chat_type === 'user'): ?>
                                    <span class="chat-header-status text-success" id="typing-indicator" style="display: none;">Typing... <i class="fas fa-pen fa-xs ms-1"></i></span>
                                    <span class="chat-header-status text-muted" id="online-indicator">Online</span>
                                <?php else: ?>
                                    <span class="chat-header-status text-success" id="typing-indicator" style="display: none;">Typing... <i class="fas fa-pen fa-xs ms-1"></i></span>
                                    <span class="chat-header-status text-muted" id="online-indicator">Group Members Active</span>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        if ($chats) {
                            ...
                            ?>
                                <div class="message-wrapper <?php echo $is_me ? 'me' : 'them'; ?>">
                                        ...
                                        <div class="d-flex align-items-center <?php echo $is_me?'justify-content-end':''; ?>" style="<?php echo $is_me?'margin-right:0px;':''; ?>">
                                            ...
                                            <span class="message-time mb-0" style="padding:0;">
                                                ...
                                                <?php if($is_me && $chat_type === 'user'): ?>
                                                    <?php 
                                                        $has_read = isset($chat['is_read']) ? $chat['is_read'] : 1; 
                                                    ?>
                                    ...
                                    <?php if ($is_me): 
                                        $my_name = isset($_SESSION['user_name']) ? $_SESSION['user_name'] : "Admin";
                    ...
                    <div class="d-flex flex-column align-items-center justify-content-center h-100 text-muted" style="background: #f8fafc;">
                        <div class="bg-white rounded-circle d-flex justify-content-center align-items-center mb-4 shadow-sm" style="width: 120px; height: 120px;">
                            <img src="https://ui-avatars.com/api/?name=Admin&background=1e293b&color=fff" style="width: 60px; height: 60px; border-radius: 50%;">
                        </div>
                        <h4 class="text-slate-800 fw-bold" style="color: #1e293b;">Welcome to Admin Messages</h4>
                        <p style="color: #64748b;">Select an employee or group to start chatting</p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="members[]" class="form-select" multiple required style="height:150px;">
                            <?php
                            $all_users = $conn->query("SELECT id, name FROM users WHERE id != $current_user_id ORDER BY name ASC");
    ...
    if(fileInput && fileNameDisplay && messageInput) {
        ...
        messageInput.addEventListener('input', function() {
            if (this.value.trim().length > 0) {
                ...
                if (onlineIndicator) onlineIndicator.style.display = 'inline';
            }
        });
    }
